...
def buildDependency() {
  try {
    ...
    # Install Nodejs
    tar -zxf ~/arise-node-Linux-x86_64.tar.gz
...
def startArise() {
  try {
    sh '''#!/bin/bash
    cd test/arise-js/; npm install; cd ../..
    cp test/config.json test/genesisBlock.json .
    export NODE_ENV=test
    JENKINS_NODE_COOKIE=dontKillMe ~/start_arise.sh
    '''
  } catch (err) {
    currentBuild.result = 'FAILURE'
    error('Stopping build, Arise failed')
  }
}

lock(resource: "Arise-Core-Nodes", inversePrecedence: true) {
