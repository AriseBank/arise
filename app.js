...
/**
 * Main entry point.
 * Loads the arise modules, the arise api and run the express server as Domain master.
...
/**
 * The config object to handle arise modules and arise api.
	...
	async.auto({
		...
		listen: ['ready', function (scope, cb) {
			scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
				scope.logger.info('Arise started: ' + scope.config.address + ':' + scope.config.port);

				if (!err) {
					if (scope.config.ssl.enabled) {
						scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
							scope.logger.info('Arise https started: ' + scope.config.ssl.options.address + ':' + scope.config.ssl.options.port);
		...
		if (err) {
			 ...
			 * @property {Object} ed - Crypto functions from arise node-sodium.
			 ...
			 * @property {string} public - Path to arise public folder.
