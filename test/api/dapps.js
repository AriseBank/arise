...
before(function (done) {
	// Send to ARISE to account 1 address
	setTimeout(function () {
		var randomARISE = node.randomARISE();
		var expectedFee = node.expectedFee(randomARISE);
...
before(function (done) {
	// Send to ARISE to account 2 address
	setTimeout(function () {
		var randomARISE = node.randomARISE();
		var expectedFee = node.expectedFee(randomARISE);
...
describe('PUT /api/dapps/withdrawal', function () {
	...
	beforeEach(function (done) {
		...
		var keys = node.arise.crypto.getKeys(randomAccount.password);
		var recipientId = node.arise.crypto.getAddress(keys.publicKey);
		var transaction = node.arise.transaction.createTransaction(randomAccount.address, 100000000, account.password);
